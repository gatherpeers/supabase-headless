...
- Grant and enable RLS as one unit for every table you expose. Tables in `public` carry no default grants, so a new table stays invisible to the Data API until granted — but once granted, a table with RLS forgotten is fully readable by that role. See [db/README.md](./db/README.md).
